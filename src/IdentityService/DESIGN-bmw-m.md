...
- Use `{colors.danger}` only for validation text, error borders, destructive confirmation, and status icons. It is never a CTA fill or brand accent.
- Keep identity forms centered or in a restrained two-column grid. Do not replace missing photography with gradients, shadows, textures, or decorative illustration.
- All new Identity UI components must reference the YAML tokens above. Extend this file before adding a new raw color, spacing, radius, or motion value.
...
- The validation, danger, dialog, toast, table, disabled, and loading tokens in
  the Identity UI profile are Revora product extensions rather than values
  extracted from the BMW M reference.
